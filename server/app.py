...
# Create a dictionary to store the students' data
students = {}

# Create a dictionary to store the questions and answers data
questions = {}

# Define a constant for the number of days after which a question becomes overdue
OVERDUE_DAYS = 7
...
# Route for login; creating and retrieving studnets
@app.route('/login', methods=['GET', 'POST'])
def login():
    if request.method == 'POST' and request.args.get("form_name") == "login_form":
...
def create_or_get_user(name):
    # If the request method is GET, return the student's data if it exists, otherwise return an error message
    if request.method == 'GET':
        if name in students:
            return jsonify(students[name])
        else:
            return jsonify({"message": "Student not found"})
    # If the request method is POST, create a new student with the given name and initialize their data
        ...
        if name not in students:
            students[name] = {
                ...
                "questions": 0,
                "responses": 0,
                "role": "student" # add a role attribute to distinguish between students and teachers
            }
            return jsonify({"message": "Student created"})
        else:
            return jsonify({"message": "Student already exists"})

@app.route('/Dashboard/<string:name>', endpoint='Dashboard')
def dashboard(name):
    ...
    if request.method == 'GET':
        if question_id in questions:
            ...
            if question["correct"] is not None:
                ...
                # Get the date and time when the question was posted
...
@app.route('/Dashboard/<string:name>', methods=['POST', 'PUT'])
def question(name):
...
@app.route('/Dashboard/<string:name>', methods=['PUT'])
def reward(name):
...
def response_rate(name):
    ...
    if request.method == 'GET':
        if name in students:
            # Calculate the response rate by dividing the responses by the questions, and multiplying by 100
            response_rate = students[name]["responses"] / students[name]["questions"] * 100
            # Round the response rate to the nearest integer
